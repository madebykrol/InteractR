...
- For RabbitMQ notifications, the inlet should derive subscriptions, queue bindings, and address usage from convention-based notification addresses rather than dedicated routing configuration. The inlet creates queues and subscriptions, both inlet and outlet declare exchanges if needed, and the outlet only publishes to exchanges while tests may use a temporary queue subscriber to observe outlet output.
- Inlet subscriptions should only be registered for notification types that have INotificationHandler registrations; event type existence alone should not auto-subscribe inlets.
...
- Inject metadata resolution directly as `INotificationMetaDataResolver` into Hub, rather than configuring it through Hub options.
- Prefer subscription registration API shaped as `registry.RegisterSubscription<NotificationType, HandlerType>()` and use that for inlet queue subscription discovery.