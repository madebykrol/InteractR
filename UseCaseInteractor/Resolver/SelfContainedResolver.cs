...
namespace UseCaseMediator.Resolver
{
    public sealed class SelfContainedResolver : IResolver, IRegistrator
    {
        ...
        public void Register<TRequest, TResponse>(IUseCaseInteractor<TRequest, TResponse> interactor) where TRequest : IUseCaseRequest<TResponse>
        {
            if (_interactors.ContainsKey(typeof(IUseCaseInteractor<TRequest, TResponse>)))
                return;

            _interactors.Add(typeof(IUseCaseInteractor<TRequest, TResponse>), interactor);
        }
    }
}
